...
/// PAYMENT SCREEN WITH UPI_PAY PACKAGE
/// Flow:
/// 1. User clicks UPI app → Creates pending transaction
/// 2. UPI app opens → User completes payment
/// 3. App gets response → Updates backend
/// 4. Shows success/failure dialog
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  Future<bool> _createPendingTransaction(String transactionRef) async {
    ...
    try {
      ...
      if (success) {
        _transactionCreated = true;
        log("✅ Transaction created with PENDING status");
        log("⏰ Will auto-fail if not confirmed within 5 minutes");
        return true;
      } else {
        log("❌ Failed to create transaction");
        return false;
  ...
  /// ✅ STEP 2: Open UPI app and initiate payment
  Future<void> _initiateUpiPayment(ApplicationMeta app) async {
    ...
    try {
      ...
      log("   Approval Ref: ${response.approvalRefNo}");
      log("══════════════════════════════════════════");
  ...
  /// ✅ STEP 3: Handle UPI response and update backend
  Future<void> _handleUpiResponse(UpiTransactionResponse response) async {
    try {
      String status;
      String upiResponse;

      // Check UPI response status
      if (response.status == UpiTransactionStatus.success) {
        status = 'success';
        upiResponse = 'Payment successful - TxnID: ${response.txnId}';
        log("✅ UPI Payment SUCCESS");
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildInstruction(
                            '1',
                            'Click on your UPI app button above',
                          ),
                          _buildInstruction(
                            '2',
